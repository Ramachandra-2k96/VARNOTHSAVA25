...
export default function PodiumCard({
  ...
  const config = {
    1: {
      ...
      cardBg: "from-yellow-900/40 to-gray-800/80",
    ...
    2: {
      translateX: "-translate-x-[120px] sm:-translate-x-[140px] md:-translate-x-[180px]",
      zIndex: "z-10",
  ...
  return (
    <motion.div
      ...
      className={`absolute ${config.zIndex} ${config.translateX}`}
    >
      <div className="flex flex-col items-center">
        ...
        <div
          className={`relative ${config.avatarSize} rounded-full bg-gradient-to-br ${config.bgColor} flex items-center justify-center mb-2 border-4 ${config.borderColor} shadow-lg overflow-hidden`}
        >
          {photoURL ? (
            <Image
              src={photoURL}
        ...
        <div
          className={`${config.cardSize} bg-gradient-to-b ${config.cardBg} backdrop-blur-md rounded-t-lg flex flex-col items-center justify-center p-2 border-t-4 ${config.cardBorder}`}
        >
